...
>
  <span className="text-base">+</span> Meeting
          ...
          meetings.map((item) => (
            ...
            >
              <div className="flex-1">
                ...
                {item.meetLink && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-sm sm:text-base text-blue-600 hover:underline mt-2 inline-block"
                  >
                    Join Meeting →
                  </a>
                )}
              ...
              <div className="mt-3 md:mt-0 text-right flex-shrink-0 md:ml-4">
                ...
                <span className="text-xs sm:text-sm bg-blue-100 text-blue-600 px-2 py-1 rounded-full mt-2 inline-block">
                  Upcoming
                </span>
              </div>
            </div>
      ...
      {/* Modal */}
      {showModal && (
        <MeetingModal onClose={() => setShowModal(false)} onSuccess={handleSuccess} />
      )}
    </div>
  );
};
